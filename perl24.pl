...
sub overwrite_file {
    ...
    print $fh $content;
    close($fh);
    print "Signal Cleared: $path\n";
}

# 1. FIX: cms-bridge.ts (Removing unused 'app')
overwrite_file('src/boot/cms-bridge.ts', <<'EOF');
import { boot } from 'quasar/wrappers';

// We leave the arguments empty to satisfy the 'no-unused-vars' rule 
// until we are ready to register global WaterMap/HealthCard components.
export default boot(() => {
  console.log("PON.INK: CMS Bridge Initialized - Signal Clear.");
});
EOF

# 2. FIX: FigureSampler.vue (Explicit Interface & Types)
overwrite_file('src/components/FigureSampler.vue', <<'EOF');
<template>
  <div class="q-pa-md">
...
print "\n--- Linting errors should now be suppressed. Restarting Dev... ---\n";
